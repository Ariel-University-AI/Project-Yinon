...
<style>
  ...
  /* ── Verdict boxes — entry animation from scale(0.95) not scale(0) ──── */
  .verdict-good {
    background: #EAF7EE;
    ...
    text-align: center;
    margin: 16px 0;
    box-shadow: 0 2px 12px rgba(26,158,63,0.12);
  ...
  .verdict-bad {
    ...
    text-align: center;
    margin: 16px 0;
    box-shadow: 0 2px 12px rgba(217,83,79,0.12);
    animation: verdictIn 280ms var(--ease-out) both;
  }
  /* Nothing in the real world appears from nothing — start from 0.95, not 0 */
  @keyframes verdictIn {
  ...
  [data-testid="stVerticalBlockBorderWrapper"] > div {
    ...
    /* GPU-friendly: box-shadow + border-color + transform only */
  ...
  @media (hover: hover) and (pointer: fine) {
    [data-testid="stVerticalBlockBorderWrapper"] > div:hover {
      box-shadow: 0 8px 32px rgba(0,106,255,0.14) !important;
      border-color: #006AFF !important;
      transform: translateY(-3px) !important;
    }
  }

  /* ── Equal height — no flex-grow animation (not GPU-accelerated) ─────── */
  ...
  /* ── Stagger entry for the 3 feature cards on the home page ──────────── */
  ...
  [data-testid="stHorizontalBlock"]:first-of-type
    ...
    [data-testid="stVerticalBlockBorderWrapper"] {
    animation: cardIn 320ms var(--ease-out) 60ms both;
  }
  [data-testid="stHorizontalBlock"]:first-of-type
    [data-testid="column"]:nth-child(3)
    [data-testid="stVerticalBlockBorderWrapper"] {
    animation: cardIn 320ms var(--ease-out) 120ms both;
  }
  @keyframes cardIn {
  ...
  /* ── Buttons — press feedback + correct easing ───────────────────────── */
  .stButton > button {
    border-radius: 7px !important;
    font-weight: 500 !important;
    /* Specific properties, ease-out for immediate feedback */
    transition: background-color 150ms var(--ease-out),
                box-shadow 150ms var(--ease-out),
                transform 100ms var(--ease-out) !important;
  }
  /* Tactile press: scale(0.97) — instant, confirms the UI heard you */
  ...
  .stButton > button[kind="primary"] {
    ...
    box-shadow: 0 2px 8px rgba(0,106,255,0.28) !important;
  }
  @media (hover: hover) and (pointer: fine) {
    ...
    .stButton > button[kind="primary"]:hover {
      ...
      box-shadow: 0 6px 18px rgba(0,106,255,0.38) !important;
      transform: translateY(-1px) !important;
    }
  }
  ...
  /* ── Expanders — subtle hover ─────────────────────────────────────────── */
  [data-testid="stExpander"] {
    transition: box-shadow 150ms var(--ease-out) !important;
  }
  @media (hover: hover) and (pointer: fine) {
